...
%     stTrans.nEleNum     = stTrans.nEleNum_x * stTrans.nEleNum_y;
    ...
    nFc             = stTrans.nCenterFreq; % [Hz]
    nFs             = stTrans.nCenterFreq * 4; % [Hz]
    nUpSampFactor   = 8;
...
%     nLag = round(((numel(aPulseSeq)+numel(aTransImpulsResp)-1)+numel(aTransImpulsResp)-1)/2); % for received signal
    ...
    for pwidx = 1:numel(aTxAngle_trc)
        ...
        if(bShowPressureField) 
            ...
            figure;
            for idx = 1:nSample
                imagesc(aX*1e3, aZ*1e3, squeeze(db(vP_deci(:,:,idx)/max(max(vP_deci(:,:,idx))))));
                title(['t = ' num2str(round(tidx/nFs*1e6*100)/100) ' us, tidx = ' num2str(idx)]);
        ...
        if(bShowSyntheticPattern)
            ...
            figure;
            tidx = round(aFocalPointPos_org(3)/nSoundSpeed*nFs);
            mVeiwPlane = vP(:,:,tidx);
            imagesc(aX*1e3, aZ*1e3, squeeze(db(mVeiwPlane/max(mVeiwPlane(:)))));
            title(['pwidx = ' num2str(pwidx) ', t = ' num2str(round(tidx/nFs*1e6*100)/100) ' us, tidx = ' num2str(tidx)]);
            axis equal; axis tight; xlabel('x [mm]'); ylabel('z [mm]');
        end
    ...
    vIntensity_t = permute(reshape(mIntensity, nSample, numel(aX), numel(aZ)),[3,2,1]);
    
    %% Export
    stBeamField.aFocalPointPos      = aFocalPointPos;
    stBeamField.aX                  = aX;
    stBeamField.aZ                  = aZ;
    stBeamField.aT                  = 0:1/nFs:(1/nFs)*(nSample-1);
    stBeamField.vIntensity_t        = vIntensity_t;
    stBeamField.nTime_at_Focus_sec  = aFocalPointPos_org(3)/nSoundSpeed;
    stBeamField.nTime_at_Focus_px   = round(aFocalPointPos_org(3)/nSoundSpeed*nFs);
